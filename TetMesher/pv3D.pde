...
class vec {
  ...
  String toString() {
    return String.format("V(%.2f, %.2f, %.2f)", this.x, this.y, this.z);
  }
}  // end class vec
...
class pt {
  ...
  String toString() {
    return String.format("P(%.2f, %.2f, %.2f)", this.x, this.y, this.z);
  }
}
...
pt P(float a, pt A, float b, pt B, float c, pt C, float d, pt D, float e, pt E) {
  return A(P(a, A, b, B), P(c, C, d, D, e, E));
}  // aA+bB+cC+dD+eE
pt P(pt P, vec V) {
...
pt Bezier(pt A, pt B, pt C, float t) {
...
float dot(pt A, pt B, pt C) {
  return d(V(A, B), V(A, C));
}  // AB*AC dot product
float det2(vec U, vec V) {
  return -U.y*V.x+U.x*V.y;
}  // U:V det product of (x,y) components
float det3(vec U, vec V) {
...
float mixed(vec U, vec V, vec W) {
  return m(U, V, W);
}  // U*(VxW)
float n2(vec V) {
  return sq(V.x)+sq(V.y)+sq(V.z);
}  // V*V    norm squared, same as d(V, V)
float n(vec V) {
  return sqrt(n2(V));
}  // ||V||  norm
float norm(vec V) {
...
}  // returns rotated version of Q by angle(CP,CR) parallel to plane (C,P,R)
